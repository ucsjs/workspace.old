...
export class Parser {
    ...
    async export(){
        //Imports
        let scriptText = `/** 
    Auto-generated by UCS.js (https://ucsjs.io)
    
    This file was generated automatically by the system, 
    I recommend that it not be changed manually because there 
    is a risk of breaking the flow, if any functionality does 
    not have the expected behavior I suggest checking the flow 
    that was parsed and in case of using third-party Blueprints, 
    enter contact the respective owner support.

    Andre Ferreira <andre@ucsjs.io>
...
{{imports}}`;
        ...
        if(this._metadata.metadata){
            for(let key in this._metadata.metadata){
                switch(key){
                    case "singleton": scriptText += `\tprivate __singleton = ${this._metadata.metadata[key]};\n`; break;
        ...
        if(this._metadata.items){
            scriptText += `\tasync exec(args?: any){\n`;
            ...
            for(let key in this._metadata.items){
                ...
                scriptText += `\t\t\t"${componentKey}": new ${this._metadata.items[key].namespace}({ args${(hasInputs) ? ', ...' + JSON.stringify(newDefaults) : ''} }),\n`;   
            }
            ...
            if(this._metadata.connections){
                for(let connection of this._metadata.connections){
                    ...
                    if(connection.to.input.includes("-trigger")){
                        ...
                        scriptText += `\t\tflow.get("${componentFrom}").subscribe("${connection.from.input.split("-")[0]}", (value) => {
            if(value !== null)
                flow.get("${componentTo}")?.trigger();
        });\n\n`;
                    }
            ...
            scriptText += `\t\tawait flow.start();\n\n`;
            scriptText += `\t\tLogger.log("Exec blueprint", "${this._namespace}")\n`;
...
export default ${this._namespace};\n\n`;
    ...
    getInput(component, inputId){
        if(inputId.includes("pv-"))
            inputId = inputId.replace("pv-", "");
    ...
    async resolve(namespace){
        ...
        for(let file of files){
            ...
            const regex = new RegExp(`export class ${namespace} extends Blueprint`, "g");
