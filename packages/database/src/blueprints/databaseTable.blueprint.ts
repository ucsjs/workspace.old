...
export class DatabaseSchemaBlueprint extends Blueprint{
    //Metadata
    ...
    private __TypeDatabase_Table: object = { color: "#6d0000" };
    private __type = ["String", "Number", "Date", "Buffer", "Boolean", "Mixed", "ObjectId", "Array", "Decimal128", "Map", "Schema"];

    public _table: string = "";
    public _columns: object = {name: "string", type: "string", primary: "boolean", index: "boolean", unique: "boolean", multi: true}; 
        ...
        this.input("connection", TypeDatabase.Connection, null, (connectionName: string) => {
            if(connectionName){
                ...
                if(this.root.hasOwnProperty(connectionName)){
                    ...
                    const repository = this.root[connectionName]?.getRepository(this.root[`${collectionClassName}Table`]);
